...
app.post("/add-lecture", async (req, res) => {
  try {
    ...
    await lecturesCollection.insertOne({
      startDate,
      endDate,
      startTime,
      endTime,
...
app.put("/update-lecture/:id", async (req, res) => {
  try {
    ...
    if (result.modifiedCount === 0)
      return res
        .status(404)
        .json({ message: "Lecture not found or no changes made" });
...
app.post("/parents", async (req, res) => {
  try {
    ...
    const { name, contact, email, address, relationship, profilePic } =
      req.body;
    const parentsCollection = db.collection("parents");

    const parent = await parentsCollection.insertOne({
      name,
      contact,
      email,
      address,
    ...
    res.status(201).json({
      message: "Parent profile added successfully",
...
app.post("/add-book", async (req, res) => {
  try {
    ...
    const { title, author, edition, subject, isbn, publisher, quantity } =
      req.body;
    ...
    res
      .status(201)
      .json({ message: "Book added successfully", bookId: newBook.insertedId });
  } catch (error) {
    res.status(500).json({ message: "Server error", error: error.message });
  }
});
...
app.put("/update-book/:id", async (req, res) => {
  try {
    ...
    await booksCollection.updateOne(
      { _id: new ObjectId(id) },
      { $set: { title, author, edition, subject, isbn, publisher, quantity } }
    );
...
app.post("/grades", async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "Grade added successfully", grade: gradeObj });
  } catch (error) {
...
app.delete("/grades/:id", async (req, res) => {
  ...
  try {
    const deletedGrade = await db
      .collection("grades")
      .deleteOne({ _id: new MongoClient.ObjectID(gradeId) });
...
app.post("/payments", async (req, res) => {
  const { studentId, studentName, paymentStatus, paymentMethod, amount } =
    req.body;
  ...
  try {
    ...
    const linkedParents = await Promise.all(
      parents.map(async (parent) => {
        const student = await db
          .collection("students")
          .findOne({ _id: new ObjectId(parent.studentId) });
